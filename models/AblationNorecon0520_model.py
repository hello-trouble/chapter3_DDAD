...
class AblationNoRecon0520Model(BaseModel):
    def name(self):
        return 'ablationnorecon0520model'
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument("--lambda_Invgg", type=float, default=0.0,help='Ablation for half-cyclic network')
...
####### changed as below:
    ...
    def forward(self):
        ...
        _,_,_,self.real_syn_dehazing_img = self.netR_Dehazing(self.real_clahe_img,data_type='real_syn')
        _,_,_,self.real_dehazing_img= self.netR_Dehazing(self.real_haze_img, data_type='real')
    ...
    def forward3(self):
        ...
        _, _, self.real_syn_features, self.real_syn_dehazing_img = self.netR_Dehazing(self.real_clahe_img, data_type='real_syn')
    ...
    def backward_G_stage1(self):
        ...
        self.loss_recon = self.criterionRecon(self.rs_recon_img,self.real_clahe_img)*lambda_recon

        lambda_Invgg = self.opt.lambda_Invgg
        self.loss_recon_Invgg = self.criterionVGG(self.rs_recon_img,self.real_clahe_img,data_type='real')*lambda_Invgg
        ...
        self.loss_G = self.loss_Syn_Dehazing+self.loss_Syn_vgg+ self.loss_R_syn_Dehazing_DC + self.loss_R_syn_Dehazing_TV + self.loss_recon +self.loss_recon_Invgg
    ...
    def backward_G_stage2(self):
            ...
            lambda_Invgg = self.opt.lambda_Invgg
            self.loss_recon_Invgg = self.criterionVGG(self.rs_recon_img, self.real_clahe_img,data_type='real') * lambda_Invgg
            self.loss_G = self.loss_Syn_Dehazing +self.loss_Syn_vgg+ self.loss_R_syn_Dehazing_DC + self.loss_R_syn_Dehazing_TV + self.loss_recon + self.loss_G_un + self.loss_recon_Invgg
            self.loss_G.backward()
    ...
    def backward_G_stage3(self):
        ...
        lambda_Invgg = self.opt.lambda_Invgg
        self.loss_recon_Invgg = self.criterionVGG(self.rs_recon_img, self.real_clahe_img,data_type='real') * lambda_Invgg
        self.loss_G = self.loss_Syn_Dehazing  +self.loss_Syn_vgg+ self.loss_R_syn_Dehazing_DC + self.loss_R_syn_Dehazing_TV + self.loss_recon + self.loss_G_un +self.loss_real_Dehazing +  self.loss_real_Dehazing_feature+self.loss_recon_Invgg
        self.loss_G.backward()
    ...
    def backward_D_depth(self):
        ...
        pred_fake_Dun = self.netD_A(self.real_dehazing_img.detach())
