...
class BaseModel():
    ...
    def initialize(self, opt):
        ...
        # self.device = torch.device('cuda:0' if torch.cuda.is_available() else torch.device('cpu')) #self.gpu_ids[0]
    ...
    def setup(self, opt, parser=None):
        if self.isTrain:
            self.schedulers = [networks_ori.get_scheduler(optimizer, opt) for optimizer in self.optimizers]
    ...
    #
    #     if int(which_epoch)<=self.opt.niter:
    #         self.model_names=['R_Dehazing','R_recon']
    #     elif int(which_epoch)>self.opt.niter:
    #         self.model_names= ['R_Dehazing','D_A','D_Rfeat','D_P','R_recon' ]
    #
    #     for name in self.model_names:
    #         if isinstance(name, str):
    ...
    def load_networks(self, which_epoch):
        if int(which_epoch)<=self.opt.niter:
            ...
            self.model_names= ['R_Dehazing','D_A','R_recon','D_Rfeat']
        # self.model_names = ['R_student']  #
        for name in self.model_names:
            if isinstance(name, str):
