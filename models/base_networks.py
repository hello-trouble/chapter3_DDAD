...
class Encoder_MDCBlock1(torch.nn.Module):
    ...
    def forward(self, ft_l, ft_h_list):
        ...
        if self.mode == 'iter2':
            ft_fusion = ft_l #down sampled feature, also  the feature map of this layer
            for i in range(len(ft_h_list)):#i=0
                ft = ft_fusion
                for j in range(self.num_ft - i):#
                    ft = self.up_convs[j](ft)
                ft = ft - ft_h_list[i] #residual, the feature of this layer (down_sampled feature ),upsampled and sub the former feature before downsampled,
                for j in range(self.num_ft - i):
                    # print(j)
                    ft = self.down_convs[self.num_ft - i - j - 1](ft) # then down sample the residual
                ft_fusion = ft_fusion + ft # then sum the feature of this layger
