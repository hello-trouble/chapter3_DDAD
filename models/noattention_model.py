...
### changed the update learning rate  with epoch instead of opt.which_epoch
import os
# import numpy as np
import torch
...
from networks_msbdn.MSBDN_DFF_noattention import Net as Net_domain_concat
...
class noattentionModel(BaseModel):
    def name(self):
        return 'noattentionmodel'
...
####### changed as below:
            ...
            self.loss_D_fake_un = torch.tensor(0.0, dtype=torch.float32).to(self.device)
            self.loss_R_syn_Dehazing_DC = torch.tensor(0.0, dtype=torch.float32).to(self.device)
    ...
    def backward_G_stage1(self):
        ...
        self.loss_G = self.loss_Syn_Dehazing+self.loss_Syn_vgg+ self.loss_R_syn_Dehazing_TV + self.loss_recon
    ...
    def backward_G_stage2(self):
            ...
            self.loss_G = self.loss_Syn_Dehazing +self.loss_Syn_vgg + self.loss_R_syn_Dehazing_TV + self.loss_recon + self.loss_G_un
            self.loss_G.backward()
    ...
    def backward_G_stage3(self):
        ...
        self.loss_G = self.loss_Syn_Dehazing  +self.loss_Syn_vgg + self.loss_R_syn_Dehazing_TV + self.loss_recon + self.loss_G_un +self.loss_real_Dehazing +  self.loss_real_Dehazing_feature
        self.loss_G.backward()
