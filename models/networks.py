...
def get_scheduler(optimizer, opt):
    ...
    if opt.lr_policy == 'linear':
        ...
        scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.0001, max_lr=0.00015,cycle_momentum=False)
    else:
...
class VGGLoss(nn.Module):
    def __init__(self):
        ...
        self.weights = [1.0/3, 1.0/3, 1.0/3, 0, 0]
        # self.weights = [0, 0, 1.0/8, 1.0/4, 1.0]
        self.instancenorm = nn.InstanceNorm2d(512, affine=False)
    def forward(self, x, y,data_type): #,data_type
        ...
        if data_type=="syn":
            ...
            x_vgg_relu5 = x_vgg[-1]
            y_vgg_relu5 = y_vgg[-1]
            return torch.mean((self.instancenorm(x_vgg_relu5) - self.instancenorm(y_vgg_relu5)) ** 2)

    # def forward_instance(self,x,y):
    #     x_vgg =self.vgg(x)
    #     y_vgg=self.vgg(y)
    #     x_vgg_relu5=x_vgg[-1]
    #     y_vgg_relu5 = y_vgg[-1]
